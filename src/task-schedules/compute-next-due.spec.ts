...
const CASES: Array<[string, ScheduleForDue]> = [
  ...
  ['WEEKLY_DAY', { cycleType: 'WEEKLY_DAY', cycle: 0, cycleDay: 2, cycleNth: null }],
  ['MONTHLY_DATE', { cycleType: 'MONTHLY_DATE', cycle: 0, cycleDay: 15, cycleNth: null }],
  ['MONTHLY_DATE last-day', { cycleType: 'MONTHLY_DATE', cycle: 0, cycleDay: 0, cycleNth: null }],
  ['QUARTERLY', { cycleType: 'QUARTERLY', cycle: 0, cycleDay: 0, cycleNth: null }],
  ['YEARLY', { cycleType: 'YEARLY', cycle: 0, cycleDay: 10, cycleNth: 6 }],
  ['MONTHLY_WEEKDAY', { cycleType: 'MONTHLY_WEEKDAY', cycle: 0, cycleDay: 1, cycleNth: 2 }],
];
...
describe('compute-next-due — due dates are always midnight (date-only)', () => {
  ...
  it.each(CASES)('computeNextDue(%s) returns midnight from a mid-day base', (_, schedule) => {
    expect(isMidnight(computeNextDue(midDayBase, schedule))).toBe(true);
  });

  it.each(CASES)('computeFirstDue(%s) returns midnight from a mid-day base', (_, schedule) => {
