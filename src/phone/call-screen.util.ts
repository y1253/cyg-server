 ...
 * A `<Number>` noun rings a personal mobile. If that mobile is off, or the call is declined,
 * the CARRIER's voicemail answers — and `<Dial>` counts an answer as an answer. Without a
 * keypress in between, the customer is dropped into a member of staff's personal voicemail:
 * the company voicemail never runs, the row is filed as answered, and the message is
 * somewhere nobody in this application can ever see it. A voicemail robot does not press 1.
 *
 ...
 * somebody who thought better of it — ends THIS LEG ONLY. The `<Dial>` goes on ringing every
 * browser and still falls through to `voice/dial-status`, i.e. to the COMPANY's voicemail.
 *
